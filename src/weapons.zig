...
pub fn stick(slot: ComponentSystem.EntityHandle) !ComponentSystem.EntityHandle {
    const entity = try ComponentSystem.Entity.create(slot.id);
    const renderer = try RenderSystem.add_renderer(.{ .texture = wepons_texture, .rect = rl.Rectangle{.x=167,.y=107,.width=8,.height=17}, .pivot = .{.x = 3, .y = 12 }});
