...
pub const World = struct {
    ...
    const ArbiterHashMapContext = struct {
        ...
        pub fn hash(self: ArbiterHashMapContext, key: ArbiterKey) u32 {
            _ = self;
            return @intCast(@intFromPtr(key.body1) ^ @intFromPtr(key.body2));
        }
    };
        ...
        for (self.bodies.values(), 0..) |*bi, i| {
            ...
            for (self.bodies.values()[i + 1 ..]) |*bj| {
                ...
                if (newArb.numContacts > 0) {
                    const prev = self.arbiters.getPtr(key);
                    if (prev) |arb| {
...
pub const Body = struct {
    ...
    pub fn init(pos: Vec2, w: Vec2, m: f32, f: f32) Body {
        ...
        if (result.mass < std.math.floatMax(f32)) {
            ...
            result.invMass = 0.0;
            result.inertia = std.math.floatMax(f32);
...
pub fn flipFeaturePair(fp: *FeaturePair) void {
    swap(u8, &fp.e.inEdge1, &fp.e.inEdge2);
    swap(u8, &fp.e.outEdge1, &fp.e.outEdge2);
}
...
pub fn clipSegmentToLine(vOut: *[2]ClipVertex, vIn: [2]ClipVertex, normal: Vec2, offset: f32, clipEdge: u8) i32 {
    ...
    if (distance0 <= 0.0) {
        vOut[@intCast(numOut)] = vIn[0];
        numOut += 1;
    }
    if (distance1 <= 0.0) {
        vOut[@intCast(numOut)] = vIn[1];
        numOut += 1;
    }
    ...
    if (distance0 * distance1 < 0.0) {
        ...
        vOut[@intCast(numOut)].v = vIn[0].v.add(vIn[1].v.sub(vIn[0].v).mulF32(interp));
        if (distance0 > 0.0) {
            vOut[@intCast(numOut)].fp = vIn[0].fp;
            vOut[@intCast(numOut)].fp.e.inEdge1 = clipEdge;
            vOut[@intCast(numOut)].fp.e.inEdge2 = EdgeNum_NO_EDGE;
        } else {
            vOut[@intCast(numOut)].fp = vIn[1].fp;
            vOut[@intCast(numOut)].fp.e.outEdge1 = clipEdge;
            vOut[@intCast(numOut)].fp.e.outEdge2 = EdgeNum_NO_EDGE;
        }
        numOut += 1;
    }
...
pub fn collide(contacts: *[MAX_POINTS]Contact, bodyA: *const Body, bodyB: *const Body) i32 {
    ...
    var i: i32 = 0;
    while (i < 2) : (i += 1) {
        const contactSeparation = dot(frontNormal, clipPoints2[@intCast(i)].v) - front;

        if (contactSeparation <= 0) {
            contacts[@intCast(numContacts)].separation = contactSeparation;
            contacts[@intCast(numContacts)].normal = normal;
            // slide contact point onto reference face (easy to cull)
            contacts[@intCast(numContacts)].position = clipPoints2[@intCast(i)].v.sub(frontNormal.mulF32(contactSeparation));
            contacts[@intCast(numContacts)].feature = clipPoints2[@intCast(i)].fp;
            if (axis == .FACE_B_X or axis == .FACE_B_Y)
                flipFeaturePair(&contacts[@intCast(numContacts)].feature);
            numContacts += 1;
        }
    }
...
pub const Arbiter = struct {
    ...
    pub fn update(self: *@This(), world: *World, newContacts: [MAX_POINTS]Contact, numNewContacts: i32) void {
        ...
        while (i < numNewContacts) : (i += 1) {
            var cNew = &newContacts[@intCast(i)];
            var k: isize = -1;
            var j: isize = 0;
            while (j < self.numContacts) : (j += 1) {
                var cOld = &self.contacts[@intCast(j)];
            ...
            if (k > -1) {
                var c = &mergedContacts[@intCast(i)];
                const cOld = &self.contacts[@intCast(k)];
                c.* = cNew.*;
                ...
                mergedContacts[@intCast(i)] = newContacts[@intCast(i)];
            }
        }

        i = 0;
        while (i < numNewContacts) : (i += 1)
            self.contacts[@intCast(i)] = mergedContacts[@intCast(i)];

        self.numContacts = numNewContacts;
    }

    pub fn preStep(self: *@This(), world: *World, inv_dt: f32) void {
        const k_biasFactor: f32 = if (world.positionCorrection) 0.2 else 0.0;
